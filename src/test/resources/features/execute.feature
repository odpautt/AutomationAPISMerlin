@test
Feature: Sanity Apis Merlin Json

  yo como que quiero realizar el sanity de la semilla

  @Caso1
  Scenario: se requiere ejecutar Cancelacion por solicitud
  ...
  @Caso2
  Scenario: se requiere ejecutar Reconexion 430
    Given se hace la peticion del token
    When se ejecuta la api configurado para la ejecucion de la automatizacion caso dos
    Then Se obtiene la respuesta de la ejecucion de la automatizacion

  @Caso3
  Scenario: Realizar Cancelación de roaming Linea Postpago 5,2
    Given se hace la peticion del token
